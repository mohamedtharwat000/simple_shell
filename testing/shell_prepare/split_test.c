...
size_t count_tokens(char *str, char *delim)
{
	size_t i = 0, count = 0, dlen = strlen(delim);
	...
	while (str[i])
	{
		if (strncmp(&str[i], delim, dlen) == 0)
		{
			...
			while (strncmp(&str[i], delim, dlen) == 0)
			{
				i += dlen;
			}
			if (strncmp(&str[i - dlen], delim, dlen) == 0)
			{
				i--;
			}
		}
...
int main(void)
{
  ...
  char *string = "  1  2  3  ";
  char *delimeters = " ";
  size_t count = count_tokens(string, delimeters);
  printf("%lu\n", count);
  words = split(string, delimeters);
